...
public class HelpController {
	 ...
	 * 添加寻物信息 
	 * */
	@RequestMapping(value = "/addFindThingHelp", method= RequestMethod.GET)
	public void addFindThingHelp(
			@RequestParam(value="userId", 		required=true) int userId,
			@RequestParam(value="name",   		required=true) String  name,
			@RequestParam(value="content",		required=true) String  content,
			@RequestParam(value="area",   		required=true) int area,
		...
		try{
			//容错，先不写测试时再说
			 this.helpService.addFindThingHelp(userId, name, content, area, site, longitude, latitude, rewardPoints, fileArray);
		}
		catch (Exception e) {
			result = SystemDef.OPER_FAIL;
		}
	 ...
	 * 更新寻物信息(发布者自己)
	 * */
	@RequestMapping(value = "/updateFindThingHelp", method= RequestMethod.GET)
	public void updateFindThingHelp(
			@RequestParam(value="helpId", 		required=true) int helpId,
			@RequestParam(value="name",   		required=false) String name,
			@RequestParam(value="content",		required=false) String content,
			@RequestParam(value="area",   		required=false) Integer area,
	 ...
	 * 获取寻物列表
	 * */
	@RequestMapping(value = "/queryFindThingList", method = RequestMethod.GET)
	public void queryFindThingList(
		...
		response.setCharacterEncoding("utf-8");
		//定义为required=false.不传参数时看报错，网上建议用基本类型定义参数，测试时试一下 
		JSONArray json = this.helpService.queryFindThingList(nowPage, pageSize);
		String result = "";
		if(json != null) result = json.toString();
	 ...
	 * 刷新寻物列表
	 * */
	@RequestMapping(value="/refreshFindThingList", method= RequestMethod.GET)
	public void refreshFindThingList(
			@RequestParam(value="helpId", 	  required=true) int helpId,
			HttpServletResponse response) throws IOException{
		response.setCharacterEncoding("utf-8");
		JSONArray json = this.helpService.refreshFindThingList(helpId);
		String result = "";
		if(json != null) result = json.toString();
	 ...
	 * 获取寻物详细信息
	 * */
	@RequestMapping(value = "/queryFindThingDetail", method = RequestMethod.GET)
